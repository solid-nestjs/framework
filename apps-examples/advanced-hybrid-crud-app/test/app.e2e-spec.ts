...
describe('Advanced Hybrid CRUD App (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    // Clean data before each test (SQL Server, PostgreSQL, and MySQL only - SQLite creates fresh DB)
  ...
  describe('Suppliers REST API', () => {
    ...
    describe('Bulk Operations', () => {
      // Skip bulk operations with camelCase WHERE clauses in PostgreSQL and MySQL due to TypeORM limitation
      const skipIfPostgresOrMysql = (process.env.DB_TYPE === 'postgres' || process.env.DB_TYPE === 'mysql') ? it.skip : it;
      
      skipIfPostgresOrMysql('DELETE /suppliers/bulk/remove-by-email - should bulk soft remove suppliers by email', async () => {
      ...
      skipIfPostgresOrMysql('PATCH /suppliers/bulk/recover-by-email - should bulk recover suppliers by email', async () => {
      ...
      skipIfPostgresOrMysql('DELETE /suppliers/bulk/delete-by-email - should bulk hard delete suppliers by email', async () => {
      ...
      skipIfPostgresOrMysql('should handle bulk operations with no matching records', async () => {
      ...
      skipIfPostgresOrMysql('should handle validation errors for bulk operations', async () => {
