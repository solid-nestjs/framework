...
import { StringFilter, NumberFilter, DateFilter } from '../../classes/inputs';
...
/**
 * Creates a WhereFields class for filtering entities with both REST and GraphQL decorators.
 ...
 * @returns A dynamically generated Where class with REST and GraphQL decorators
 *
 * @example
 * ```typescript
...
export function createWhereFields<T>(
  ...
  >) {
    try {
      ...
      if (isPlainField) {
        ...
        applyDecoratorToProperty(
          Field(() => plainType, {
            nullable: true,
            description: parsedConfig.description || `Filter by ${fieldName}`,
            deprecationReason: parsedConfig.deprecated
      ...
      throw new Error(
        `Error processing field '${fieldName}': ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
