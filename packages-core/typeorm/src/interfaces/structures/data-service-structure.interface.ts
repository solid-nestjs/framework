...
export type RelationsConfig<EntityType> = RelationsConfigBase<EntityType> | FindOptionsRelations<EntityType>;

export function getMainAliasFromConfig<EntityType>(relationsConfig?: RelationsConfig<EntityType>): string | undefined {
    const conf = relationsConfig as any;

    if (conf?.mainAlias === undefined && conf?.relations === undefined)
        return undefined;
    else
        return conf?.mainAlias;
}

export function getRelationsFromConfig<EntityType>(relationsConfig?: RelationsConfig<EntityType>): Relation[] | FindOptionsRelations<EntityType> | undefined {
    const conf = relationsConfig as any;

    if (conf?.mainAlias === undefined && conf?.relations === undefined)
        return conf;
    else
        return conf?.relations;
}
